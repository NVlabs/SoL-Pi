...
The extension creates no sidecar, attestation, payload-capture, or research-instrumentation files. State entries are never injected into the model context as entries; the generic post-compaction reminder is, and a boundary compaction restates the progress fields the model itself supplied to `update_plan` in the compaction instruction, bounded to 4 KiB and drawn from the same conversation the compaction is summarizing. Deleting the Pi session removes both kinds of persisted Online Context Compact data.
