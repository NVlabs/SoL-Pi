...
The extension creates no sidecar, attestation, payload-capture, or research-instrumentation files. State entries are never injected into the model context as entries; the generic post-compaction reminder is, and a boundary compaction can restate progress fields from current or restored session state. Those fields are treated as untrusted evidence, JSON encoded inside explicit delimiters, and preceded by an instruction that their contents must never be followed as instructions, commands, or links. The complete compaction instruction is bounded to 4 KiB. Deleting the Pi session removes both kinds of persisted Online Context Compact data.
